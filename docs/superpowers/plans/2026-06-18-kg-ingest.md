...
Add the first three v1 goals to `kg_ingest/builders/goals.py`: **Dragon scimitar** (`item:4587` — 60 Attack + Monkey Madness I completed), **fairy rings** (`access:fairy-rings` — Fairytale II - Cure a Queen `in_progress` chain), **Tzhaar-ket-om / obby maul** (`item:6528` — 60 Strength; item name casing `Tzhaar-ket-om` per data/items_equipment.json, M3). Each goal contributes a goal node, one `requires` edge (`dst=None`), and an AND `cond_group` of the locked atoms (K3/§6.1-6.2). Requirements are wiki-verified per K8 and hardcoded here. `build_goals()` returns `tuple[list[Node], list[Edge], dict[int, ConditionGroup]]`.

**K8/K1 gates-only stance (single bought items):** single tradeable items are gated by prerequisites only (skills/quests); owning the final item is the deferred acquisition/cost layer. The goal node IS the item node (`item:<id>`), requiring only its prerequisite gates — no self-loop since the requires group references skill/quest nodes, NOT the item node itself. No ITEM atom, no separate `gear_loadout` node for single-item goals. Multi-piece sets (Infinity) and assemblies (Voidwaker) still require owning their component items (the components are the gate). Barrows gloves (`item:7462`) follows this shape exactly (`item:7462` requires only `quest:recipe-for-disaster`); Dragon scimitar and obby maul now follow the same shape.

**Self-loop clarification:** there is NO self-loop because the requires group for `item:4587` references `skill:attack` and `quest:monkey-madness-i` — NOT `item:4587` itself. `store.requires_dag()` only adds cond_dep edges from an owner to the `ref_node` inside its group, and those ref_nodes are distinct from the goal node. `find_cycles()` therefore detects no cycle.
...
- [x] **Step 1: Write the failing test (RED).** Create `tests/kg_ingest/test_goals_part1.py`:
  ```python
  ...
  # so they do NOT appear in _SUPPORT_NODES.
  _SUPPORT_NODES = [
      Node(id="quest:monkey-madness-i", kind=NodeKind.QUEST,
           name="Monkey Madness I", slug="monkey-madness-i", data={}),
  ...
  def test_build_goals_emits_item_goal_nodes():
      ...
      nodes, edges, groups = build_goals()
      node_ids = {n.id for n in nodes}
      assert "item:4587" in node_ids
      assert "item:6528" in node_ids
  ...
  def test_scimitar_goal_has_no_item_atom():
      ...
      assert not [c for c in group.children
...
- [x] **Step 2: Implement `build_goals()` part 1 (GREEN).** Create `kg_ingest/builders/goals.py` with the K8/K1 gates-only shape: Dragon scimitar goal node = `item:4587` (NodeKind.ITEM, requires AND of skill:attack >= 60 boostable + quest:monkey-madness-i completed); Obby maul goal node = `item:6528` (NodeKind.ITEM, requires AND of skill:strength >= 60 boostable). No ITEM atom on either goal. No separate `gear_loadout` nodes. Fairy rings unchanged (`access:fairy-rings`, requires Fairytale II in_progress).

  Run (GREEN): `./venv/bin/python -m pytest tests/kg_ingest/test_goals_part1.py -q` → `13 passed`.

  **DO NOT edit the engine here.** `store.requires_dag()`/`find_cycles()` stay byte-identical to `main`; the only engine change on this branch is Task 1's K5 (state.py + conditions.py). The gates-only item-node model means there is no self-loop — and the Task 8 acyclicity check MUST remain able to catch any future self-loop.

- [x] **Step 3: Full suite regression.** `./venv/bin/python -m pytest -q` → `0 failed` (246 passed; the 2 "not owned" tests removed and 3 new structural/no-item-atom tests added). No engine diff outside K5 + json_store.py.

- [x] **Step 4: Commit.**
  ```
  git add kg_ingest/builders/goals.py tests/kg_ingest/test_goals_part1.py docs/superpowers/plans/2026-06-18-kg-ingest.md
  git commit -m "fix(kg-ingest): align scimitar/obby goals to K8/K1 gates-only (item node, no ownership atom, no separate node)"
  ```
