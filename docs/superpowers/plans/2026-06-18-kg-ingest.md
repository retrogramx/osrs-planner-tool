...
- [ ] **Step 2: Implement `build_goals()` part 1 (GREEN).** Create `kg_ingest/builders/goals.py`:
  ...
  def build_goals() -> tuple[list[Node], list[Edge], dict[int, ConditionGroup]]:
      ...
      return nodes, edges, groups
  ```
  Run (GREEN): `./venv/bin/python -m pytest tests/kg_ingest/test_goals_part1.py -q` → `10 passed`.

- [ ] **Step 3: Full suite regression.** `./venv/bin/python -m pytest -q` → `0 failed` (173 full-suite baseline intact + 10 new goal tests in `test_goals_part1.py` + sibling-task tests). If lower than expected, STOP and use superpowers:systematic-debugging.
...
- [ ] **Step 4: Commit.**
  ...
  First three v1 goals: Dragon scimitar (item:4587 = own + 60 Attack + Monkey
  Madness I completed), fairy rings (access:fairy-rings = Fairytale II - Cure a
  Queen in_progress), Tzhaar-ket-om obby maul (item:6528 = own + 60 Strength).
  Each emits a goal node + a dst=None requires edge whose cond_group is an AND of
  the locked atoms (K3/§6.1-6.2). Builder-local deterministic _group_id/_edge_id
  (owner_id, slot) helpers (reused by Task 6). Engine-level met/unmet acceptance.
