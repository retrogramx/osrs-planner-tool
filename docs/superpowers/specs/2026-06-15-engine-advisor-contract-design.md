...
Step{
  ...
  node_id: str | None,               # None for ref-less atoms (quest_points, combat_achievement_points, combat_level, clue_scrolls)
  ...
  set_ref: str | None,               # for clue_scrolls
...
```
referenced_atoms: list[{ atom_type, ref_node: NodeId|None, threshold|qty: int|None,
                         state: "not_started"|"in_progress"|"completed"|None,
                         currency: str|None, amount: int|None, is_partial: bool }]
```
`is_partial` (for `quest`: required `state < completed`) lets the check catch "fully complete FT2" when the real atom is a partial `state >= in_progress`.

---

## 6. Account state & three-valued (Kleene) evaluation

**Problem:** a two-valued evaluator coerces absent data (an untracked boss KC / clue count below its Hiscores cutoff, unobservable `quest`/`achievement_diary`/`item`) to `False`, producing a confident wrong "locked — go train X." This is the launch-common case for public accounts (ADR-0004).
...
**Manual confirmation (first-class, not an edge case).** The public Hiscores list an activity (boss KC, clue/minigame score) only once the account clears that activity's **population-dependent tracking cutoff**, so an absent score could mean *0* or *below cutoff* — and quests/diaries/per-task CAs/items aren't on the Hiscores at all. So `indeterminate` is the *common* launch state, not a corner case. (Per-skill **levels/XP are the exception** — present for any tracked account even when the skill's rank shows `--`.) When the Engine returns a `cant_verify` blocker, the UI surfaces a one-tap *"confirm this value"* prompt; the answer is written as a `manual` fact — trusted per §9.3 and never clobbered by a later Hiscores sync (monotonic metrics still reconcile via `max(old,new)`, so a stale manual value can't hold back a higher synced one). The richest long-term fix is the companion RuneLite plugin (§9.6), which reads true in-game state the Hiscores cannot expose; manual input is the fallback for users without the plugin.

### 6.4 Observability table (what each source can see)
The Kleene evaluator decides *Known* vs `UNKNOWN` per condition-atom by consulting the **authoritative observability table in [ADR-0004](../../../decisions/2026-06-13-0004-public-multi-account.md#observability-table-per-condition-atom-family--source)** (validated against real Hiscores lookups, 2026-06-18). The engine's `observable_families` set (above) is built from it. Load-bearing facts that table pins down:
- **Cardinal rule — Hiscores absence ≠ zero.** An absent activity score (boss KC, clue/minigame count) is `UNKNOWN`, not `0`: it may simply be below that activity's population-dependent appear-cutoff. Only `manual` or `plugin` can disambiguate.
- **Public Hiscores expose:** per-skill level+XP (for any tracked account, even when rank is `--`), activity scores once tracked (boss KC, clue tiers, CA points, minigames), the collections-logged **count** (never *which* slots), and **account type** (by which board ranks them). They do **not** expose `combat_level` (derived), `quest_points`/`quest`, `achievement_diary`, per-task `combat_achievement`, `item`, or `clog_slot`.
- **Plugin (claimed)** is the only public-account source for quests/diaries/per-task CAs/pets (**core**), bank contents + banked XP (**bank**, §9.7), and obtained collection-log slots (**clog**). `manual` is the no-plugin fallback; `derived` atoms (`combat_level`, `quest_points`, `access:*`) are engine-computed and inherit `UNKNOWN` from their inputs.
...
3. **Scalars** — a number stated *as a requirement/threshold* for a referenced atom must match that atom's typed scalar in `referenced_atoms` (skill_level→threshold, item→qty, quest→state, …). Pass-through numbers (KC targets, GE prices, combat level 250) are allowed; a requirement-number with no matching atom = fabricated → reject.
...
This rides the §9.3 two-writer rule (`source='plugin'`) and is an ingest-layer brick; the contract pins the shape, the brick implements it. A **second account-state stream — the collection log (obtained slots) — rides the same plugin push + two-writer rule (`source='plugin'`)**; both feed the completionist lens ([[completionist-collection-log]]).
...
7. **`resolve_goal` confidence/tie rule — confirmed v1.** Return `Resolved` iff exactly one candidate clears a high-confidence bar (exact slug/name match, or a single fuzzy match scoring a clear margin above the second). Otherwise `Ambiguous` with up to 5 candidates; `NotFound` if none clear the floor. The data foundation now supplies richer matching/disambiguation material (the `item_dictionary` + node names/aliases across 17 datasets), so candidates and each `one_line_disambiguator` are data-backed, not hand-listed.
8. **Theme search — upgraded by the data foundation.** Originally planned as just `kind_filter` + an `is_boss` flag (free-text tags deferred). The foundation changes this: themes are **derived from the structured domains at launch with zero hand-curation** — *bossing* (`bosses_pvm` + `combat_achievements`), *money-makers* (`money_making`, account-type-correct), *skilling* (skill categories), *clues*, *diaries*, *raids*, and the **collection-log categories** (Bosses/Raids/Clues/Minigames/Other). So *"get me into bossing / money-making / clues"* works day one from real category data; bespoke free-text tags become an optional later nicety, not a prerequisite. `kind_filter` still ships as the base.

**§13 now fully resolved (all 8 items) + foundation-confirmed (2026-06-17):** scale-gap **G1 (the `quest` atom's `state` field)** is real and supplied — `quests.json` models prereqs as `{quest, state: in_progress|completed}` (e.g. *Rag and Bone Man II* needs three quests only *in_progress*); the prereq/blocker model must honor `state`, never assume 'completed'. Income realization is now a **computed per-method field** (`iron_realizable_value` + `realization_channel`, §8). Observability + the two-writer source list extend to the new RuneLite **bank feed** and **collection-log** streams (§13.4, §9.7).
