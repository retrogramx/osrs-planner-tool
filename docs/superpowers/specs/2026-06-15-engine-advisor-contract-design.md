...
4. **Observability authority.** ADR-0004 (public-multi-account) is the authority for which atom families each source (`hiscores`/`plugin`/`manual`/`inferred`) can observe; the per-family table the Kleene logic depends on is derived from it. *(Action: confirm/extend ADR-0004 with the explicit table.)* **Extended (post-foundation):** new observable streams — the **RuneLite bank feed** (§9.7: bank contents + banked XP + High-Alch value) and the **collection-log** (obtained slots) — must be added to ADR-0004's table as `plugin` sub-streams; they fill the largest Hiscores blind spots.
...
**§13 now fully resolved (all 8 items) + foundation-confirmed (2026-06-17):** scale-gap **G1 (`quest_stage`)** is real and supplied — `quests.json` models prereqs as `{quest, stage: started|completed}` (e.g. *Rag and Bone Man II* needs three quests only *started*); the prereq/blocker model must honor `stage`, never assume 'completed'. Income realization is now a **computed per-method field** (`iron_realizable_value` + `realization_channel`, §8). Observability + the two-writer source list extend to the new RuneLite **bank feed** and **collection-log** streams (§13.4, §9.7).
