...
5. **Quest prereq STATE — "Started:" = partial-completion.** A prereq like `Started:Waterfall Quest` means *started, not completed* — a real OSRS nuance modeled by the **`quest`** atom's `state` field, which carries the ordered enum `not_started|in_progress|completed` (the wiki "Started:" convention maps to `in_progress`); G1 scale-gap in the contract spec. Model prereqs as `{quest, state: in_progress|completed}`, not a bare name.
