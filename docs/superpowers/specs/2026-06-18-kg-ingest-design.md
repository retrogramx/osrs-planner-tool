...
A requirement flagged `ironman: true` applies to **every account that must self-source** — ALL non-main accounts: standard ironman, HCIM, GIM, HCGIM, and UIM. Only a main (who can buy/trade) skips it. Model it as a plain 2-child OR (no NOT group):
> `OR( account_type == "main",  <the requirement> )`

- A **main** → `account_type == "main"` is TRUE → the `OR` is satisfied → the requirement is **invisible**
  to mains (honors the "never explain a main's restrictions to them" rule).
- An **ironman-family** account (standard + HCIM + GIM + HCGIM) → `account_family(mode) == "main"` is FALSE → reduces to the real requirement.
- A **UIM** → `account_family(mode) == "uim"` ≠ `"main"` → FALSE → reduces to the real requirement. UIM is the most restricted account and must self-source.

This is a plain 2-child OR group: child 1 = a `ConditionAtom(atom_type=account_type, data={"value":"main"})`; child 2 = the real requirement atom (e.g. the skill_level atom). No NOT group is needed. `account_type == "main"` here is a **family** test (§6.4): `account_family(state.mode)` is compared to `"main"` for all iron/UIM variants and correctly returns FALSE.
...
- The §6.3 wrapper keys on the **`main` family**, so all non-main accounts — `ironman` family (standard + HCIM +
  GIM + HCGIM) AND `uim` — fail the `account_type == "main"` test and have the iron requirement applied. No
  account type is incorrectly exempted.
...
and UIM's **no-bank storage** constraint (affects how a UIM holds/uses items — i.e. item *storage/strategy* —
not whether a self-sourcing requirement is met; UIM *does* see iron requirements via the §6.3 wrapper).
...
4. Account-family-aware iron requirements work correctly for all account types: **main** (req invisible) / **ironman** / **HCIM** / **GIM** / **HCGIM** / **UIM** (all six non-main variants see the requirement applied).
...
GE-price cache · GIM intra-group-trade edge case · UIM no-bank storage constraint (affects item storage/strategy, not self-sourcing requirements — iron reqs apply to UIM via the §6.3 wrapper).
