...
- **Value each output for the family.** Coins → face value (both). **main** → GE price × qty/hr (`provider.ge_price`). **ironman/uim** → the **best realization in coins**: `max(` High-Alch of the raw drop, **process-then-realize** `)`, where process-then-realize **walks the multi-step chain** for the drop (e.g. green dragonhide → **tan** [Tanner service fee] → leather → **craft** [Crafting level] → d'hide body → **High-Alch 4,680**), **subtracting internal costs** (tan fee, secondary inputs like thread) and **gated by the account's skills**. **v1 over-count (disclosed, post-review):** every output carrying a High-Alch value is valued at it — *including items an iron would realistically use for XP/consumables, not alch:* **Prayer fodder** (dragon bones alch for ~96 but are worth far more as gilded-altar Prayer XP), **ensouled heads** (reanimation), and **herblore secondaries** (grimy herbs are cleaned + used, not alched). So an iron gp/hr is a small over-count in the *gold* direction and omits those items' true (XP) value. Doing it right — bones → Prayer XP with **gold contribution 0** — needs the deferred `{gold, xp, resources}` accounting + a "non-income fodder" tag (a v1→v2 follow-up). The earlier wording "bones → Prayer don't count as income" was aspirational; v1 does not yet exclude them.
