...
`{item, qty, tradeable, members, condition?, choice_group?}`. `item` **references `items_equipment.json`** for stats / equip-requirements / value (thin overlay — we never re-derive). Kinds: weapon / armour / tool / misc / coins / gem. **`tradeable` is read per account type (the iron lens):** a *tradeable* reward a main can simply buy on the GE (so the reward just saves GP), whereas an iron must self-obtain it; an *untradeable* item can't be bought or traded by anyone, so it must come from its in-game source — and for quest-exclusive untradeables (e.g. Barrows gloves, Ava's device) that source *is* the quest, making it the sole acquisition path. The flag is referenced from `items_equipment.json` and feeds the iron-vs-main realization logic. `condition` / `choice_group` per §3.6.
